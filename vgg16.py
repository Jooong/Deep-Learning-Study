...
conv1 = tf.nn.conv2d(X, W1, strides=[1,1,1,1],padding="SAME")    ; print(conv1)#  224*224 size의 그림 * 32개
relu1 = tf.nn.relu(conv1)   ; print(relu1)
pool1 = tf.nn.max_pool(relu1, ksize=[1,2,2,1],strides=[1,2,2,1],padding="SAME") ; print(pool1) # 112*112의 그림 * 32개

# layer 2
W2 = tf.Variable(tf.random_normal([3,3,32,64],stddev=0.01))    ; print(W2)  # 3*3 filter가 64개
conv2 = tf.nn.conv2d(pool1, W2, strides=[1,1,1,1],padding="SAME")    ; print(conv2)#  112*112 size의 그림 * 64개
relu2 = tf.nn.relu(conv2)   ; print(relu2)
pool2 = tf.nn.max_pool(relu2, ksize=[1,2,2,1],strides=[1,2,2,1],padding="SAME") ; print(pool2) # 56*56의 그림 * 64개

# layer 3
W3 = tf.Variable(tf.random_normal([3,3,64,128],stddev=0.01))    ; print(W3)  # 3*3 filter가 128개
conv3 = tf.nn.conv2d(pool2, W3, strides=[1,1,1,1],padding="SAME")    ; print(conv3)#  56*56 size의 그림 * 128개
relu3 = tf.nn.relu(conv3)   ; print(relu3)
pool3 = tf.nn.max_pool(relu3, ksize=[1,2,2,1],strides=[1,2,2,1],padding="SAME") ; print(pool3) # 28*28의 그림 * 128개

# layer 4
W4 = tf.Variable(tf.random_normal([3,3,128,256],stddev=0.01))    ; print(W4)  # 3*3 filter가 256개
conv4 = tf.nn.conv2d(pool3, W4, strides=[1,1,1,1],padding="SAME")    ; print(conv4)#  28*28 size의 그림 * 256개
relu4 = tf.nn.relu(conv4)   ; print(relu4)
pool4 = tf.nn.max_pool(relu4, ksize=[1,2,2,1],strides=[1,2,2,1],padding="SAME") ; print(pool4) # 14*14의 그림 * 256개

# layer 5
W5 = tf.Variable(tf.random_normal([3,3,256,512],stddev=0.01))    ; print(W5)  # 3*3 filter가 512개
conv5 = tf.nn.conv2d(pool4, W5, strides=[1,1,1,1],padding="SAME")    ; print(conv5)#  14*14 size의 그림 * 512개
relu5 = tf.nn.relu(conv5)   ; print(relu5)
pool5 = tf.nn.max_pool(relu5, ksize=[1,2,2,1],strides=[1,2,2,1],padding="SAME") ; print(pool5) # 7*7의 그림 * 512개
flattened = tf.reshape(pool5, [-1, 7*7*512])

# fully connected
dense1 = tf.layers.dense(flattened,4096,activation=tf.nn.relu)
drop1 = tf.nn.dropout(dense1,0.5)

dense2 = tf.layers.dense(drop1,4096,activation=tf.nn.relu)
drop2 = tf.nn.dropout(dense2,0.5)

logit = tf.layers.dense(drop2,2,activation=tf.nn.softmax)
